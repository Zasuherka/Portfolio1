...
class FoodWrap extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final String userTitle =
        food.isThisFoodOnTheMyFoodList ? 'Удалить из моей еды' : 'Добавить в мою еду';
    return Wrap(
      children: [
          ...
          child: ListTile(
            ...
            onTap: () {
              food.isThisFoodOnTheMyFoodList
                  ? BlocProvider.of<FoodBloc>(context).add(FoodEvent.deleteFood(food: food))
                  : BlocProvider.of<FoodBloc>(context).add(FoodEvent.addingFood(food: food));
              Navigator.pop(context);
            },
          ),
        ),
